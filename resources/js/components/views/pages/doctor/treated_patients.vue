    ...
    <div class="d-flex">
      ...
      <div class="container">
        ...
        <div class="mb-3">
          ...
          />
          <button class="btn btn-primary" @click="searchAppointments">Search</button>
        </div>
        ...
        <table class="table table-striped">
          ...
          <tbody>
            ...
            <tr v-for="appointment in filteredAppointments.slice(0, 5)" :key="appointment.id">
  ...
  export default {
    ...
    data() {
      return {
        ...
        filteredAppointments: [], // Holds the filtered appointments for display
        doctorId: localStorage.getItem('doctorId'), // Get the doctorId from localStorage
        showToday: false,  // Set default to show all appointments
        showAll: true,     // Set default to show all appointments
        searchTerm: '',    // Holds the search term
      };
    },
    mounted() {
      this.fetchFilteredAppointments();  // Fetch all completed appointments when the component is mounted
    },
    methods: {
      ...
      fetchFilteredAppointments() {
        ...
        // Fetch appointments with treatment_status equal to 1
        axios.post(`/appointments/${this.doctorId}`)
          .then(response => {
            this.appointments = response.data
              .filter(appointment => appointment.treatment_status === 1)  // Filter by treatment_status
              .map(appointment => {
                appointment.session_date = this.formatDate(appointment.session_date);
                return appointment;
              });
      ...
      searchAppointments() {
        ...
        if (this.searchTerm) {
          this.filteredAppointments = this.appointments.filter(appointment => {
            const nameMatch = appointment.patient_name.toLowerCase().includes(this.searchTerm.toLowerCase());
            const phoneMatch = appointment.phone_number.includes(this.searchTerm);
